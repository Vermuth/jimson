  ...
  describe Client do
    ...
    describe 'sending a batch request' do
      it 'sends a valid JSON-RPC batch request and puts the results in the response objects' do
        ...
        expect(sum.success?).to be true
        expect(sum.failure?).to be false
        expect(sum.result).to eq(7)

        expect(subtract.result).to eq(19)

        expect(foo.failure?).to be true
        expect(foo.success?).to be false
    ...
    describe 'error handling' do
      context 'when an error occurs in the Jimson::Client code' do
        ...
        it 'lets you access additional data parameter on exception class' do
          ...
          response = MultiJson.encode(
            ...
            'error' => {
              'code' => -32099,
              'message' => 'Server error -32099: Something went wrong',
              'data' => { 'reason' => 'something_went_wrong' }
            },
            'id' => '1'
          )
          client_helper = ClientHelper.new(SPEC_URL)
          allow(ClientHelper).to receive(:new).and_return(client_helper)
          client = Client.new(SPEC_URL)
          expect(RestClient).to receive(:post).with(SPEC_URL, request, content_type: 'application/json').and_return(@resp_mock)
          expect(@resp_mock).to receive(:body).at_least(:once).and_return(response)
          begin
            client.sum(1, 2, 3)
          rescue Jimson::Client::Error::ServerError => e
            expect(e.message).to eq('Server error -32099: Something went wrong')
            expect(e.code).to eq(-32099)
            expect(e.data).to eq('reason' => 'something_went_wrong')
          end
        end
      end
    end
