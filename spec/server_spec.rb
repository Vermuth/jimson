...
module Jimson
  describe Server do
    ...
    class TestHandler
      ...
      class SomethingWentWrong < Jimson::Server::Error; end

      def subtract(a, b = nil)
        return a['minuend'] - a['subtrahend'] if a.is_a?(Hash)
        a - b
      end
      ...
      def uglier_method
        raise SomethingWentWrong.new(-32098, 'Something went wrong', status: :something_went_wrong)
      end
    end
      ...
      context "with 'show_errors' enabled" do
        it 'returns an error name and first line of the stack trace' do
          ...
          expect(resp).to eq(
            ...
            'error' => {
              'code' => -32099,
              'message' => "Server application error: RuntimeError at #{__FILE__}:39:in `ugly_method'",
              'data' => nil
            },
            'id' => 1
          )
    ...
    describe 'receiving a call for uglier method' do
      it 'returns only global error without stack trace' do
        req = {
          'jsonrpc' => '2.0',
          'method'  => 'uglier_method',
          'id' => 1
        }
        post_json(req)
      ...
      context 'when not all requests are notifications' do
        it 'returns an array of responses' do
          ...
          expect(resp).to eq([
            ...
            { 'jsonrpc' => '2.0', 'error' => { 'code' => -32600, 'message' => 'The JSON sent is not a valid Request object.', 'data' => nil }, 'id' => nil },
            { 'jsonrpc' => '2.0', 'error' => { 'code' => -32601, 'message' => "Method 'foo.get' not found.", 'data' => nil }, 'id' => '5' },
    ...
    describe "receiving a 'system.' request" do
      ...
      context "when the request is 'system.listMethods'" do
        it 'returns response with all jimson_exposed_methods on the handler(s) as strings' do
          ...
          expected = ['get_data', 'notify_hello', 'subtract', 'sum', 'car', 'ugly_method', 'uglier_method', 'update', 'system.isAlive', 'system.listMethods', 'other.multiply']
